...
exports.updateVaStatus = exports.getVaById = exports.getVas = exports.createVa = void 0;
...
const createVa = async (req, res) => {
    try {
        ...
        const vaExists = await va_schema_1.default.findOne({ email });
        if (vaExists) {
        ...
        const va = await va_schema_1.default.create({
            name,
            email,
        ...
        await training_progress_schema_1.default.create({
            vaId: va._id,
        ...
        await certifications_schema_1.default.create({
            vaId: va._id,
...
const getVas = async (req, res) => {
    try {
        ...
        const [vas, totalCount] = await Promise.all([
            va_schema_1.default.find(query)
                .select("-password")
                .populate("assignedAdminId", "name email")
                .skip(skip)
                .limit(limit)
                .sort({ createdAt: -1 }),
            va_schema_1.default.countDocuments(query),
        ]);
...
const getVaById = async (req, res) => {
    try {
        const { id } = req.params;
        const va = await va_schema_1.default.findById(id).select("-password").populate("assignedAdminId", "name email");
        if (!va) {
...
exports.getVaById = getVaById;
const updateVaStatus = async (req, res) => {
    try {
        const { id } = req.params;
        const { status, assignedAdminId } = req.body;
        const va = await va_schema_1.default.findById(id);
        if (!va) {
            res.status(404).json({ message: "VA not found" });
            return;
        }
...
exports.updateVaStatus = updateVaStatus;
//# sourceMappingURL=va-management-controller.js.map